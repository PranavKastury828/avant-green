...
export const CardHeavy = ({ image, text, description }) => {
 return (
    <div className="w-full rounded-lg overflow-hidden shadow-md transition duration-[400ms] group ease-in-out relative group">
      <img
        src={image.src}
        alt="Card Image"
        className="w-full h-full object-cover group-hover:scale-110 transition duration-[400ms] ease-in-out"
          />
          <div className="absolute top-0 left-0 w-full h-full bg-apres-huit/50 transition duration-300 ease-in-out"></div>
      <div className="absolute top-0 left-0 w-full h-full bg-black/50">
         <svg width="57" height="30" viewBox="0 0 57 30" xmlns="http://www.w3.org/2000/svg" className='fill-current text-interface-light group-hover:text-avant-green transition ease-in-out duration-[400ms] mr-[8px] absolute z-[100] top-2 right-2'>
        ...
        <div className='absolute bottom-3 px-[24px]'>
          <p className=" text-4xl font-medium text-avant-green mb-[40px] transition duration-300 ease-in-out hover:text-green-500">
          {text}
              </p>
          <p className={futuraFontRegular.className + ' text-xl text-centre-parfait font-thin' }>{description}</p>
        </div>
